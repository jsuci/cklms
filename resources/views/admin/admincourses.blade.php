@extends('admin.layouts.app')

@section('content')

            <div class="page-content-inner">


                <nav id="breadcrumbs" class="mb-3">
                    <ul>
                        <li><a href="#"> <i class="uil-home-alt"></i> </a></li>
                        <li> Courses </li>

                    </ul>
                </nav>

                <div class="d-flex justify-content-between mb-3">
                    <h3> Courses (14) </h3>

                    <div>
                        <a href="#" class="btn btn-default">
                            <i class="uil-plus"> </i> New course
                        </a>
                    </div>
                </div>

                <div class="uk-child-width-1-3@m uk-grid" uk-grid="" uk-scrollspy="cls: uk-animation-slide-bottom-small; target: &gt; div ; delay: 200">

                    <div class="uk-first-column uk-scrollspy-inview uk-animation-slide-bottom-small" style="">
                        <a href="#">
                            </a><div class="card animate-this uk-inline-clip"><a href="#">
                                <img src="{{asset('templatefiles/2.png')}}" alt="">
                                <div class="card-body text-center pb-3">
                                    <h6 class=" mb-0"> Learn Angular Fundamentals </h6>
                                </div>
                                </a><div class="card-footer py-0 border-top-0"><a href="#">
                                    </a><div class="row align-items-center text-center"><a href="#">
                                        </a><div class="col-6 py-3"><a href="#">
                                            </a><a href="#" class="text-muted"> 120$ Sales </a>
                                        </div>
                                        <div class="col-6 py-3">
                                            <a href="#" class="text-muted"> 20 Enrolled </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        
                    </div>

                    <div class="uk-scrollspy-inview uk-animation-slide-bottom-small" style="">
                        <a href="#">
                            </a><div class="card animate-this uk-inline-clip"><a href="#">
                                <img src="{{asset('templatefiles/3.png')}}" alt="">
                                <div class="card-body text-center pb-3">
                                    <h6 class=" mb-0"> The Complete JavaScript </h6>
                                </div>
                                </a><div class="card-footer py-0 border-top-0"><a href="#">
                                    </a><div class="row align-items-center text-center"><a href="#">
                                        </a><div class="col-6 py-3"><a href="#">
                                            </a><a href="#" class="text-muted"> 90$ Sales </a>
                                        </div>
                                        <div class="col-6 py-3">
                                            <a href="#" class="text-muted"> 8 Enrolled </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        
                    </div>

                    <div class="uk-scrollspy-inview uk-animation-slide-bottom-small" style="">
                        <a href="#">
                            </a><div class="card animate-this uk-inline-clip"><a href="#">
                                <img src="{{asset('templatefiles/1.png')}}" alt="">
                                <div class="card-body text-center pb-3">
                                    <h6 class=" mb-0"> Ultimate Web Developer Course </h6>
                                </div>
                                </a><div class="card-footer py-0 border-top-0"><a href="#">
                                    </a><div class="row align-items-center text-center"><a href="#">
                                        </a><div class="col-6 py-3"><a href="#">
                                            </a><a href="#" class="text-muted"> 230$ Sales </a>
                                        </div>
                                        <div class="col-6 py-3">
                                            <a href="#" class="text-muted"> 21 Enrolled </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        
                    </div>

                    <div class="uk-grid-margin uk-first-column uk-scrollspy-inview uk-animation-slide-bottom-small" style="">
                        <a href="#">
                            </a><div class="card animate-this uk-inline-clip"><a href="#">
                                <img src="{{asset('templatefiles/5.png')}}" alt="">
                                <div class="card-body text-center pb-3">
                                    <h6 class=" mb-0"> Ultimate Web Developer Course </h6>
                                </div>
                                </a><div class="card-footer py-0 border-top-0"><a href="#">
                                    </a><div class="row align-items-center text-center"><a href="#">
                                        </a><div class="col-6 py-3"><a href="#">
                                            </a><a href="#" class="text-muted"> 142$ Sales </a>
                                        </div>
                                        <div class="col-6 py-3">
                                            <a href="#" class="text-muted"> 13 Enrolled </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        
                    </div>

                    <div class="uk-grid-margin uk-scrollspy-inview uk-animation-slide-bottom-small" style="">
                        <a href="#">
                            </a><div class="card animate-this uk-inline-clip"><a href="#">
                                <img src="{{asset('templatefiles/7.png')}}" alt="">
                                <div class="card-body text-center pb-3">
                                    <h6 class=" mb-0"> Bootstrap Introduction </h6>
                                </div>
                                </a><div class="card-footer py-0 border-top-0"><a href="#">
                                    </a><div class="row align-items-center text-center"><a href="#">
                                        </a><div class="col-6 py-3"><a href="#">
                                            </a><a href="#" class="text-muted"> 80$ Sales </a>
                                        </div>
                                        <div class="col-6 py-3">
                                            <a href="#" class="text-muted"> 6 Enrolled </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        
                    </div>

                    <div class="uk-grid-margin uk-scrollspy-inview uk-animation-slide-bottom-small" style="">
                        <a href="#">
                            </a><div class="card animate-this uk-inline-clip"><a href="#">
                                <img src="{{asset('templatefiles/2.png')}}" alt="">
                                <div class="card-body text-center pb-3">
                                    <h6 class=" mb-0"> Learn Angular Fundamentals </h6>
                                </div>
                                </a><div class="card-footer py-0 border-top-0"><a href="#">
                                    </a><div class="row align-items-center text-center"><a href="#">
                                        </a><div class="col-6 py-3"><a href="#">
                                            </a><a href="#" class="text-muted"> 120$ Sales </a>
                                        </div>
                                        <div class="col-6 py-3">
                                            <a href="#" class="text-muted"> 20 Enrolled </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        
                    </div>

                    <div class="uk-grid-margin uk-first-column uk-scrollspy-inview uk-animation-slide-bottom-small" style="">
                        <a href="#">
                            </a><div class="card animate-this uk-inline-clip"><a href="#">
                                <img src="{{asset('templatefiles/3.png')}}" alt="">
                                <div class="card-body text-center pb-3">
                                    <h6 class=" mb-0"> The Complete JavaScript </h6>
                                </div>
                                </a><div class="card-footer py-0 border-top-0"><a href="#">
                                    </a><div class="row align-items-center text-center"><a href="#">
                                        </a><div class="col-6 py-3"><a href="#">
                                            </a><a href="#" class="text-muted"> 90$ Sales </a>
                                        </div>
                                        <div class="col-6 py-3">
                                            <a href="#" class="text-muted"> 8 Enrolled </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        
                    </div>

                    <div class="uk-grid-margin uk-scrollspy-inview uk-animation-slide-bottom-small" style="">
                        <a href="#">
                            </a><div class="card animate-this uk-inline-clip"><a href="#">
                                <img src="{{asset('templatefiles/1.png')}}" alt="">
                                <div class="card-body text-center pb-3">
                                    <h6 class=" mb-0"> Ultimate Web Developer Course </h6>
                                </div>
                                </a><div class="card-footer py-0 border-top-0"><a href="#">
                                    </a><div class="row align-items-center text-center"><a href="#">
                                        </a><div class="col-6 py-3"><a href="#">
                                            </a><a href="#" class="text-muted"> 230$ Sales </a>
                                        </div>
                                        <div class="col-6 py-3">
                                            <a href="#" class="text-muted"> 21 Enrolled </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        
                    </div>

                    <div class="uk-grid-margin uk-scrollspy-inview uk-animation-slide-bottom-small" style="">
                        <a href="#">
                            </a><div class="card animate-this uk-inline-clip"><a href="#">
                                <img src="{{asset('templatefiles/5.png')}}" alt="">
                                <div class="card-body text-center pb-3">
                                    <h6 class=" mb-0"> Ultimate Web Developer Course </h6>
                                </div>
                                </a><div class="card-footer py-0 border-top-0"><a href="#">
                                    </a><div class="row align-items-center text-center"><a href="#">
                                        </a><div class="col-6 py-3"><a href="#">
                                            </a><a href="#" class="text-muted"> 142$ Sales </a>
                                        </div>
                                        <div class="col-6 py-3">
                                            <a href="#" class="text-muted"> 13 Enrolled </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        
                    </div>

                </div>


                <ul class="uk-pagination my-5 uk-flex-center" uk-margin="">
                    <li class="uk-active uk-first-column"><span>1</span></li>
                    <li><a href="#">2</a></li>
                    <li><a href="#">3</a></li>
                    <li class="uk-disabled"><span>...</span></li>
                    <li><a href="#"><span uk-pagination-next="" class="uk-icon uk-pagination-next"><svg width="7" height="12" viewBox="0 0 7 12" xmlns="http://www.w3.org/2000/svg" data-svg="pagination-next"><polyline fill="none" stroke="#000" stroke-width="1.2" points="1 1 6 6 1 11"></polyline></svg></span></a></li>
                </ul>





                <!-- footer
                ================================================== -->
                <div class="footer">
                    @include('admin.inc.footer')
                </div>

            </div>

        @endsection